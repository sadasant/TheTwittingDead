  ...
  $.fn.twitterFriends = function (allOptions) {
    ...
    return this.each(function () {
      ...
      var requestUsers = function () {
        ...
        var data = {
          screen_name: op.username,
          cursor: -1
        };
        $.ajax({
          url: url,
          data: data,
          success: requestedUsers,
          dataType: "jsonp",
          cache: true
        })
      };
      var requestedUsers = function (json) {
        if (!json.users) {
          if (op.debug) {
            div.html('<b style="color:red">Error:' + (json.error ? json.error : "unkown") + "</b>")
          }
          return
        }
        rs = json.users;
        ...
        if (rs.length > op.users_max) {
          rs.length = op.users_max
        }
        rs = rs.reverse();
        output();
        vp = -1;
      ...
      var addUser = function (x, i) {
        ...
        $('<a style="display:none;height:' + op.user_image + 'px" href="' + u + '" title="' + t + '"><img src="' + x.profile_image_url + '" border="0" height="' + op.user_image + '" width="' + op.user_image + '"/></a>').appendTo(usrsDiv)
      };
      ...
      var linkify = function (d) {
        ...
        }).replace(/\B\@([A-Z0-9_]{1,15})/gi, '@<a class="tf-at" href="http://twitter.com/$1">$1</a>').replace(/\B\#([A-Z0-9_]+)/gi, '<a class="tf-hashtag" href="http://search.twitter.com/search?q=%23$1">#$1</a>')
      };
      ...
      var requestedMe = function (j) {
        ...
        if (!j.screen_name) {
          if (op.debug) {
            hdDiv.html('<b style="color:red">Error:' + (j.error ? j.error : "unkown") + "</b>")
          }
          return
        }
        hdDiv.html(op.header.replace(/_tp_/g, "http://twitter.com/" + j.screen_name).replace(/_fr_/g, j.friends_count).replace(/_fo_/g, j.followers_count).replace(/_ti_/g, j.profile_image_url))
      };
      var init = function () {
        if (op.header) {
          requestMe()
      ...
      var output = function () {
        ...
        if (op.header) {
          hdDiv = $('<div class="tf-header"></div>').appendTo(div)
        }
        usrsDiv = $('<div class="tf-users"></div>').appendTo(div);
        if (op.tweet) {
          stsDiv = $('<div class="tf-tweet"></div>').appendTo(div)
        }
      };
      init()
    })
